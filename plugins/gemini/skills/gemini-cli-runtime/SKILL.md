...
- **Timeout policy for rescue:** `task`'s built-in default is unbounded, which is unsafe for rescue because the parent agent blocks on a single foreground call. If the forwarded request includes `--timeout <duration>`, pass it through verbatim. If it does NOT include `--timeout`, you MUST add `--timeout 15m` to the `task` call. This bounds rescue against a hung Gemini process indefinitely stranding the parent agent. Forms accepted: `300s`, `5m`, `1h`, `500ms`, or `0` to disable. Users who want unbounded rescue must pass `--timeout 0` explicitly. On timeout the helper exits 124 — surface that to the user rather than retrying.
